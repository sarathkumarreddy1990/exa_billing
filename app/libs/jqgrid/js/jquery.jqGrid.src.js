...
(function ($) {
    ...
    $.fn.jqGrid = function( pin ) {
        ...
        return this.each( function() {
            ...
            for(i=0;i<this.p.colNames.length;i++){
                ...
                var i18nAttr = '';
                if(ts.p.i18nNames && ts.p.i18nNames.length == ts.p.colModel.length && ts.p.i18nNames[i] != ''){
                    i18nAttr = " i18n='"+ts.p.i18nNames[i]+"'";
                    ts.p.colNames[i] = '';
                }
                thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+"><span"+i18nAttr+">"+ts.p.colNames[i]+"</span>";
    ...
    $.jgrid.extend({
        ...
        setRowData : function(rowid, data, cssp) {
            ...
            var cp = typeof cssp;
            var t = this;
            var $colModel = $(this.p && this.p.colModel);
            var tryData = function ( data, vl, ind ) {
                var lcdata = {};
                $colModel.each(function ( i ) {
                    var nm = this.name;
                    var title;
                    if ( data[ nm ] !== undefined ) {
                        lcdata[ nm ] = this.formatter && typeof this.formatter === 'string' && this.formatter == 'date' ?
                                       $.unformat.date.call(t, data[ nm ], this) :
                                       data[ nm ];
        ...
        setCell : function(rowid,colname,nData,cssp,attrp, forceupd) {
            return this.each(function(){
                ...
                if(!$t.grid) {return;}
                if(isNaN(colname)) {
                    $t.p.colModel.some(function(obj, i){
                        if ( obj.name == colname ) {
                            pos = i;
                            return true;
                        }
...
(function($){
    ...
    $.jgrid.extend({
        ...
        GridDestroy : function () {
            function removal ( self ) {
                var $this = $(self);
                var $other = $("#gbox_" + $.jgrid.jqID(self.id));
        ...
        GridUnload : function(){
            console.time('GridUnload timer');
            var regClassRemoval = /ui-jqgrid-btable/;
            var result = this.each(function(){
                if ( !this.grid ) {return;}
                var defgrid = {id: $(this).attr('id'),cl: $(this).attr('class')};
                var $pager = $(this.p.pager);
                if (this.p.pager) {
                    $pager.empty().removeClass("ui-state-default ui-jqgrid-pager corner-bottom");
                }
                var newtable = document.createElement('table');
                newtable.id = defgrid.id;
                newtable.className = defgrid.cl.replace(regClassRemoval, '');
                var $newTable = $(newtable);
                var gid = $.jgrid.jqID(this.id);
                if( $pager.parents("#gbox_"+gid).length === 1 ) {
                    $newTable.insertBefore("#gbox_"+gid).show();
                    $pager.insertBefore("#gbox_"+gid);
                } else {
                    $newTable.insertBefore("#gbox_"+gid).show();
                }
                $(this).jqGrid('clearBeforeUnload');
            ...
            console.timeEnd('GridUnload timer');
            return result;
        },
        ...
        sortableColumns : function (tblrow)
        {
            return this.each(function (){
                ...
                if (ts.p.sortable.exclude) {
                    columnsToExclude = ts.p.sortable.exclude;
                }
                var sortable_opts = {
                    "tolerance" : "pointer",
                    "axis" : "x",
                    "scrollSensitivity": "1",
                    "items": '>th:not(:has(#jqgh_'+tid+'_cb'+',#jqgh_'+tid+'_rn'+',#jqgh_'+tid+'_subgrid,#jqgh_tblGridPS_status'+columnsToExclude+'),:hidden)',
                    "placeholder": {
                    ...
                    "update": function(event, ui) {
                        ...
                        if ($.isFunction(ts.p.sortable.update)) {
                            ts.p.sortable.update.call(permutation, ts);
                        }
                ...
                if (ts.p.sortable.exclude) {
                    //console.log(ts.p.sortable.exclude)
