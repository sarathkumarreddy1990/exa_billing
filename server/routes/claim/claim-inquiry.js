...
router.get('/claim_invoice', async (req, res) => {
    const data = await inquiryController.getInvoicePayments(req.query);
    httpHandler.sendRows(req, res, data);
});

router.get('/claim_invoice/age', async (req, res) => {
    const data = await inquiryController.getInvoicePaymentsAge(req.query);
    httpHandler.sendRows(req, res, data);
});
