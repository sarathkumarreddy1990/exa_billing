...
module.exports = {
    ...
    autoCollectionsProcess: async (params) => {
        ...
        let writeOffQuery ='';
        let selectResult = 'SELECT id, null, null FROM update_status';
        ...
        const sql = SQL`WITH
                ...
                patient_details AS (
                    ...
                    LEFT JOIN LATERAL (
                        SELECT
                            MAX(applied_dt) AS last_payment_dt
                    ...
                    WHERE
                    CASE
                        WHEN ( payment_details.last_payment_dt::DATE - last_patient_statement.created_dt ) > ${acr_claim_status_statement_days} THEN TRUE
                        WHEN ( timezone(get_facility_tz(c.facility_id::integer), now()::timestamp)::DATE - payment_details.last_payment_dt::DATE  ) > ${acr_claim_status_last_payment_days} THEN TRUE
                    ELSE FALSE
                    END
                    ORDER BY p.id ASC
                )
                ...
                , claim_patient_balance AS (
                    SELECT
                        ...
                        ,p.full_name
                        ,p.facility_id
                        ,ARRAY_AGG(claims.id) AS claim_ids
