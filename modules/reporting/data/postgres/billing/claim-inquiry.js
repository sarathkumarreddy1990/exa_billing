        ...
        claim_data AS (
            SELECT
                ...
                CASE
                  ...
                  WHEN bc.payer_type = 'service_facility_location' THEN public.get_payer_name(bc.id, bc.pos_map_id)
                  ELSE  NULL
                END AS carrier,
            ...
            INNER JOIN LATERAL (
                SELECT
                    i_bch.claim_id
                FROM
                    billing.charges i_bch
                WHERE <% print(chargeUserIds) %>
                UNION
                SELECT
                    i_bcc.claim_id
                FROM
                    billing.claim_comments i_bcc
                WHERE <% print(comentUserIds) %>
                UNION
                SELECT
                    i_bch.claim_id
                FROM
                    billing.payments i_bp
