...
-- -------------------------------------------------------------------------------------------------------------
-- Data Corrections included in Migrations script ( EXA-10277)

IF NOT EXISTS (SELECT 1 from billing.migration_log where table_name = 'data_corrections') THEN
 RAISE NOTICE 'Data corrections started';

-- -------------------------------------------------------------------------------------------------------------

RAISE NOTICE '--- 1  Data correction for Order having duplicate patient_insurance ids  ----';
-- -----------------------------------------------------------------------------------------------------------------------
...
RAISE NOTICE '--- 2 Data fix for duplicate claims. Keep the claim with last updated date as live  ----';
...
-- -------------------------------------------------------------------------------------------------------------
RAISE NOTICE '--- 3 Data fix for duplicate claims with same submission date/time  other than Electronic Billing  ----';
...
INSERT INTO  billing.migration_log (table_name,migration_dt) VALUES ('data_corrections',now());
ELSE
      RAISE NOTICE 'Billing data_corrections already finished';
END IF;
...
WHERE coalesce(insurance_provider_id,0) != 0 ;  -- EXA - 10248
